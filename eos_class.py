...
class mixtures:
    ...
    def get_logrho_sp(self, _s, _lgp, _y, _z, ideal_guess=True, arr_guess=None):
        logt = self.get_logt_sp( _s, _lgp, _y, _z, ideal_guess=ideal_guess, arr_guess=arr_guess)
        return self.get_logrho_pt(_lgp, logt, _y, _z)

    def get_logp_rhot(self, _lgrho, _lgt, _y, _z, ideal_guess=True, arr_guess=None):
    ...
    def get_logp_srho(self, _s, _lgrho, _y, _z, ideal_guess=True, arr_guess=None):
        ...
        def err(_lgp):
            # Error function for logt(S, logp)
            logrho_test = self.get_logrho_sp(_s, _lgp, _y, _z)
    ...
    def get_logt_rhop(self, _lgrho, _lgp, _y, _z, ideal_guess=True, arr_guess=None, method='root'):
        ...
        def err(_lgt):
            # Error function for logt(S, logp)
            logrho_test = self.get_logrho_pt(_lgp, _lgt, _y, _z)
            return (logrho_test/_lgrho) - 1
    ...
    #     def err(_s):
    #         # Error function for logt(S, logp)
    #         logrho_test = self.get_logrho_sp_tab(_s, _lgp, _y, _z)
    #         return (logrho_test/_lgrho) - 1
    ...
    # def inversion(self, a_arr, b_arr, y_arr, z_arr, basis):
    ...
    #                 if basis == 'sp':
    #                     res1 = self.get_logt_sp(a_const, b_const, y_const, z_arr)
    #                     res2 = self.get_logrho_pt(b_const, res1, y_const, z_arr)
    #                 elif basis == 'rhot':
    #                     res1 = self.get_logp_rhot(a_const, b_const, y_const, z_arr)
    #                     res2 = self.get_s_pt(res1, b_const, y_const, z_arr)
    #                 elif basis == 'srho':
    #                     res1 = self.get_logp_srho(a_const, b_const, y_const, z_arr)
    #                     res2 = self.get_t_sp(a_const, res1, y_const, z_arr)
    #                 elif basis == 'rhop':
    #                     res1_temp, conv = self.get_logt_rhop(a_const, b_const, y_const, z_arr, method='newton')
    #                     res1 = self.interpolate_non_converged_temperatures_1d(z_arr, res1_temp, conv, interp_kind='quadratic')
    #                     res2 = self.get_s_pt(b_const, res1_temp, y_const, z_arr)
    #                 else:
    #                     raise ValueError('Unknown inversion basis. Please chose sp, rhot, srho, or rhop')
                    ...
                    if basis == 'sp':
                        ...
                        if prev_res1_temp is None:
                            res1_temp, conv = self.get_logt_rhop(
                                a_const, b_const, y_const, z_arr, method='newton_brentq', ideal_guess=True
                            )
                        else:
                            res1_temp, conv = self.get_logt_rhop(
                                a_const, b_const, y_const, z_arr, method='newton_brentq', ideal_guess=False, arr_guess=prev_res1_temp
                            )
